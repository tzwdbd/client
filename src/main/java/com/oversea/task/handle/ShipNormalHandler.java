...
@Component
public class ShipNormalHandler implements ShipHandler {
	...
	public void handle(Task task, TaskResult taskResult,List<RobotOrderDetail> orderDetails){
		...
		String mallName = orderDetail.getSiteName();
		...
		for (int i = 0; i < RETRY_COUNT; i++){
			...
			autoBuy.setOrderNo(orderDetail.getOrderNo());
			if(autoBuy == null){
				...
				for(RobotOrderDetail detail : orderDetails){
					...
					detail.setStatus(AutoBuyStatus.AUTO_SCRIBE_SETUP_FAIL.getValue());
				}
				return;
			}else{
				logger.error("获取到autobuy:"+autoBuy.getClass().getSimpleName());
			}
			autoBuy.setOrderDetailList(orderDetails);
			...
			try{
				...
				setOrderDetailListStatus(orderDetails,status.getValue());
