...
public class AmazonUKAutoBuy extends AutoBuy {
	...
	private void valueClick(FirefoxDriver driver, String valueStr) throws Exception {
		...
		try {
			...
			TimeUnit.SECONDS.sleep(3);
			driver.findElement(By.xpath("//span[@class='a-sheet-close']")).click();
			return;
		} catch (Exception e) {
			logger.error("没有找到选sku新样式-底部弹出样式");
		}
		...
		for (int i = 0; i < 4; i++) {
			try {
				...
				String format1 = String
						.format("//span[@class='a-declarative' and contains(@data-mobile-twister, '%s')]", temp);
				By value = By.xpath(format1);
				// String format1 = String.format("//span[@class='a-declarative'
				// and contains(text(), '%s')]", valueStr);
				// By value = By.xpath(format1);
				...
				Utils.sleep(2000);
				driver.executeScript("var tar=arguments[0];var top=tar.offsetTop;window.scrollTo(0,top);",
						valueElement);
				Utils.sleep(6000);
				valueElement.click();
			} catch (Exception e) {
		...
		try {
				...
				for (int i = 0; i < skuList.size(); i++) {
					if (i % 2 == 1) {
						...
						if (i > 1) {
							...
							try {
								wait0.until(ExpectedConditions
										.visibilityOfElementLocated(By.xpath("//div[@id='a-popover-content-1']")));
								isFindNext = true;
							} catch (TimeoutException e) {
							}
							if (!isFindNext) {
								try {
									wait0.until(ExpectedConditions.visibilityOfElementLocated(
											By.xpath("//div[@class='a-popover-header-secondary']")));
									isFindNext = true;
								} catch (TimeoutException e) {
								}
							}
						...
						if (isFindNext) {
							...
							// 需要选择sku
							String format0 = String
									.format("//div[@class='a-column a-span9' and contains(text(), '%s')]", keyStr);
							...
							try {
								...
								try {
									String format1 = String.format(
											"//div[@class='a-column a-span9' and contains(text(), '%s')]",
											skuList.get(i - 1));
							...
							if (keyElement == null) {// 重新寻找keyElement
								if (!Utils.isEmpty(keyStr)) {
									int half = keyStr.length() / 2;
									if (half > 0) {
										format0 = String.format(
												"//div[@class='a-column a-span9' and contains(text(), '%s')]",
												keyStr.substring(0, half));
										...
										try {
											...
											try {
												...
												keyElement = driver.findElement(By
														.xpath("//span[@class='a-declarative' and contains(@data-mobile-twister,'"
																+ "\"label\":\"" + skuList.get(i) + "\""
																+ "')]/label"));
												newStyle = true;
											} catch (Exception ex) {
							...
							if (keyElement == null && !Utils.isEmpty(keyStr)) {
								// 再次寻找keyElement
								List<WebElement> listTemp = driver
										.findElements(By.xpath("//div[@class='a-column a-span9']"));
							...
							// 再次寻找 分词匹配
							if (keyElement == null && !Utils.isEmpty(keyStr)) {
								List<WebElement> keyList = driver
										.findElements(By.xpath("//div[@class='a-column a-span9']"));
								keyElement = findClosedWelement(keyStr, keyList);
							}

							if (keyElement == null) {
								logger.debug("找不到keyElement url= " + productUrl + "&& sku = "
										+ (skuList.get(i - 1) + ":" + skuList.get(i)));
								return AutoBuyStatus.AUTO_SKU_NOT_FIND;
							}
							Utils.sleep(2000);
							...
							if (newStyle) {
								try {
									keyElement
											.findElement(By.cssSelector("div.twister-mobile-tiles-swatch-unavailable"));
			...
			try {
				...
				try {
					...
					WebElement oneTimePurchase = selectPanel.findElement(By
							.xpath(".//span[@class='modeTitle a-text-bold' and contains(text(),'One-time Purchase')]"));
			...
			try {
				...
				try {
					WebElement pricePanel = driver
							.findElement(By.cssSelector("table#price td#priceblock_dealprice > span"));
				...
				try {
					...
					try {
						...
						wait.until(ExpectedConditions
								.visibilityOfElementLocated(By.xpath("//div[@class='a-alert-content']/div")));
						List<WebElement> list = driver
								.findElements(By.xpath("//div[@class='a-box a-spacing-base stma-line-item-details']"));
						if (list != null && list.size() > 0) {
							for (WebElement w : list) {
								try {
									...
									if (ww != null) {
										...
										if (StringUtil.isNotEmpty(text)
												&& text.contains("can't be shipped to your selected address")) {
											...
											if (we != null) {
												...
												if (StringUtil.isNotEmpty(singlePriceStr)
														&& singlePriceStr.startsWith("$")
														&& singlePriceStr.length() > 1) {
													singlePriceStr = singlePriceStr.substring(1);
													for (Map.Entry<String, String> entry : priceMap.entrySet()) {
														if (StringUtil.isNotEmpty(singlePriceStr)
																&& singlePriceStr.equals(entry.getValue())) {
															statusMap.put(entry.getKey(),
																	AutoBuyStatus.AUTO_PAY_NOT_SUPPORT_ADDRESS
																			.getValue());
														}
													}
												}
					...
					try {
						WebDriverWait wait = new WebDriverWait(driver, 15);
						WebElement gift = wait.until(ExpectedConditions.visibilityOfElementLocated(
								By.xpath("//div[@class='a-section save-gift-button-box']")));
						gift.click();
					} catch (Exception e) {
	...
	AutoBuyStatus selectDeliveryOptions() {
		try {
			try {
				WebDriverWait wait = new WebDriverWait(driver, WAIT_TIME);
				wait.until(
						ExpectedConditions.visibilityOfElementLocated(By.xpath("//form[@id='shippingOptionFormId']")));
			} catch (Exception e) {
			}
			...
			for (WebElement e : opts) {
				if (e != null) {
					...
					if (!Utils.isEmpty(text) && text.indexOf("FREE Two-Day Delivery") != -1) {
					...
					if (!Utils.isEmpty(text) && text.indexOf("FREE Standard Delivery") != -1) {
					...
					if (!Utils.isEmpty(text) && text.indexOf("FREE Delivery") != -1) {
					...
					if (!Utils.isEmpty(text) && text.indexOf("Standard Delivery") != -1) {
			...
			TimeUnit.SECONDS.sleep(2);
			try {
				driver.findElement(By.xpath("//input[@class='a-button-text' and  contains(@value, 'Continue')]"))
						.click();
			} catch (Exception e) {
				try {
					driver.findElement(By.xpath("//input[@class='a-button-input' and  contains(@value, 'Continue')]"))
							.click();
				} catch (Exception ee) {
					try {
						...
						try {
							driver.findElement(
									By.xpath("//form[@id='shippingOptionFormId']/span/span[@class='a-button-inner']"))
									.click();
						} catch (Exception e0) {
							try {
								driver.findElement(By
										.xpath("//form[@id='shippingOptionFormId']/div/span[@class='a-button-inner']"))
										.click();
							} catch (Exception e1) {
			...
			try {
				WebDriverWait wait = new WebDriverWait(driver, 10);
				wait.until(
						ExpectedConditions.visibilityOfElementLocated(By.xpath("//form[@class='a-declarative checkout-page-form a-spacing-none']")));
				try {
					driver.findElement(By.xpath("//input[@class='a-button-text' and  contains(@value, 'Continue')]"))
							.click();
				} catch (Exception e) {
					try {
						driver.findElement(
								By.xpath("//input[@class='a-button-input' and  contains(@value, 'Continue')]")).click();
					} catch (Exception ee) {
						try {
							driver.findElement(By.xpath("//input[contains(@value, 'Continue')]")).click();
						} catch (Exception eee) {
							try {
								driver.findElement(By
										.xpath("//form[@id='shippingOptionFormId']/span/span[@class='a-button-inner']"))
										.click();
							} catch (Exception e0) {
								try {
									driver.findElement(By
	...
	AutoBuyStatus selectGiftCard(String prePrice) {
		try {
			...
			wait.until(ExpectedConditions.visibilityOfElementLocated(
					By.xpath("//div[@id='select-payments-view']/div[@class='a-container']/div[@id='imb-wrapper']")));
			// WebElement wrapper =
			// wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector("div.a-container
			// form.checkout-page-form")));
			TimeUnit.SECONDS.sleep(2);
			// List<WebElement> giftBtns =
			// driver.findElements(By.xpath("//label[@class='existing-gift-card-balance']"));
			List<WebElement> giftBtns = driver.findElements(By.cssSelector(
					"form.checkout-page-form div.a-spacing-base div.a-box div.a-box-inner label.existing-gift-card-balance"));
			if (giftBtns == null || giftBtns.size() <= 0) {
				giftBtns = driver.findElements(By.xpath(
						"//label[@class='existing-gift-card-balance a-control-row a-touch-checkbox a-text-normal']"));
			}
			if (giftBtns == null || giftBtns.size() <= 0) {
				giftBtns = driver.findElements(By.xpath(
						"//label[@class='existing-gift-card-balance a-control-row a-touch-radio a-text-normal']"));
			}
			...
			if (giftBtns != null && giftBtns.size() > 0) {
						...
						if (m.find()) {
							...
							if (left > 0) {
								...
								try {
									...
									// return
									// AutoBuyStatus.AUTO_PAY_SELECT_GIFTCARD_FAIL;
									boolean isFind = true;
									try {
				...
				try {
					WebElement radio = driver.findElement(
							By.xpath("//label[@paymentmethodtype='creditCard']/i[@data-a-input-name='paymentMethod']"));
	...
	public AutoBuyStatus pay(Map<String, String> param) {
		...
		if (goodsInCart != null && goodsInCart.size() > 0) {
			...
			By submitBy = By.xpath(
					"//input[@class='a-button-text place-your-order-button' and contains(@value, 'Place your order')]");
			try {
			...
			if (orderSubmit != null) {
				...
				try {
					TimeUnit.SECONDS.sleep(5);
					By next = By.xpath(
							"//button[@class='pda_button pda_spacing_top_medium' and contains(text(),'Continue placing')]");
			...
			try {
				...
				if (isPay) {
					...
					logger.error("=================");
					// orderSubmit.click();
					orderSubmit.sendKeys(Keys.RETURN);
				} else {
	...
	String getAmazonOrderNo() {
		...
		if (flag) {
			try {
				WebDriverWait wait = new WebDriverWait(driver, WAIT_TIME);
				wait.until(ExpectedConditions
						.visibilityOfElementLocated(By.xpath("//h3[contains(text(),'View order details')]")));
				WebElement x = driver
						.findElement(By.xpath("//h3[contains(text(),'View order details')]/following-sibling::div[1]"));
	...
	public boolean gotoMainPage() {
		try {
			...
			WebDriverWait wait = new WebDriverWait(driver, WAIT_TIME);
			wait.until(ExpectedConditions
					.visibilityOfElementLocated(By.xpath("//a[@id='nav-greeting-name' and contains(text(),'Hello')]")));
			return true;
		} catch (Exception e) {
		}
		return false;
	...
	public AutoBuyStatus scribeExpress(RobotOrderDetail detail) {
		...
		try {
			...
			WebElement account = panelAccount
					.findElement(By.xpath(".//span[@class='nav-ftr-text' and contains(text(),'Your Account')]"));
			Utils.sleep(1500);
			account.click();
		} catch (Exception e) {
		...
		try {
			WebElement order = wait.until(ExpectedConditions.visibilityOfElementLocated(
					By.xpath("//span[@class='a-size-base' and contains(text(),'Your orders')]")));
			Utils.sleep(1500);
			order.click();
		} catch (Exception e) {
		...
		try {
			...
			WebElement search = wait
					.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//a[@id='showSearchBar']")));
			Utils.sleep(1500);
			search.click();
		...
		// 等待Filter orders不可见
		try {
			boolean isVisbile = wait.until(
					ExpectedConditions.invisibilityOfElementLocated(By.xpath("//input[@id='searchOrdersInput']")));
		...
		try {
			...
			if (ll != null && ll.size() > 0) {
				...
				for (WebElement w : ll) {
					WebElement ww = w.findElement(
							By.xpath(".//div[@class='item-view-left-col-inner']/a[@class='a-link-normal']"));
					if (ww != null) {
						String productLink = ww.getAttribute("href");
						if (productLink != null && productLink.contains(productEntityCode)) {
							// 被砍单
							try {
								WebElement canceled = driver.findElement(By.xpath(
										"//span[@class='a-size-small a-color-secondary' and contains(text(),'Cancelled')]"));
								if (canceled != null) {
							...
							WebElement seeShip = w.findElement(
									By.xpath(".//div[@class='a-box-inner' and contains(text(),'Track package')]"));
							if (seeShip != null) {
								isFind = true;
								seeShip.click();
								break;
		...
		try {
			wait.until(ExpectedConditions
					.visibilityOfElementLocated(By.xpath("//div[@class='ship-track-small-vertical-widget']")));
		} catch (Exception e) {
		...
		// {
		// WebElement noResult =
		// driver.findElement(By.xpath("//span[@class='a-text-bold' and
		// contains(text(),'No results found. Please try another search.')]"));
		// if (noResult != null)
		// {
		...
		// // 被砍单
		// WebElement canceled =
		// driver.findElement(By.xpath("//span[@class='a-size-small
		// a-color-secondary' and contains(text(),'Cancelled')]"));
		// if (canceled != null)
		// {
		// logger.error(mallOrderNo + " 这个订单被砍单了,需重新下单");
		// return AutoBuyStatus.AUTO_SCRIBE_ORDER_CANCELED;
		...
		// try
		// {
		// WebElement orderDetail0 =
		// driver.findElement(By.xpath("//div[@class='a-box-inner' and
		// contains(text(),'View order details')]"));
		// Utils.sleep(1500);
		// orderDetail0.click();
		// }
		// catch (Exception e)
		...
		// {
		// orderDetail =
		// wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[@class='a-box-inner'
		// and contains(text(),'Track shipment')]")));
		// Utils.sleep(1500);
		// }
		// catch (Exception e)
		// {
		...
		// driver.findElements(By.xpath("//div[@class='a-box-group']"));
		...
		// w.findElements(By.xpath(".//span[@class='a-size-base
		// a-color-base']"));
		...
		// w.findElement(By.xpath(".//a/div[contains(text(),'Track
		// shipment')]"));
		// if(subDetail != null){
		...
		// {
		// //
		// wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//span[@class='a-text-bold'
		// and contains(text(),'Carrier')]")));
		// wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//h4[contains(text(),'Order
		// Details')]")));
		// Utils.sleep(2500);
		// List<WebElement> elements =
		// driver.findElements(By.xpath("//div[@class='a-box-inner
		// a-padding-medium']"));
		// if (elements != null && elements.size() > 0)
		// {
		// for (WebElement e : elements)
		...
		// String expressCompany = str.substring(a + aStr.length(), b).trim();
		// String expressNo = str.substring(b + bStr.length()).trim();
		// data.put(AutoBuyConst.KEY_AUTO_BUY_PRO_EXPRESS_COMPANY,
		// expressCompany);
		// data.put(AutoBuyConst.KEY_AUTO_BUY_PRO_EXPRESS_NO, expressNo);
		// logger.error("expressCompany = " + expressCompany);
		// logger.error("expressNo = " + expressNo);
	...
	public AutoBuyStatus redeemGiftCard(String cardNo, String balance) {
		...
		try {
			...
			WebElement yourAccount = panel.findElement(By.xpath(
					"//li[@class='nav-li']/a[@class='nav-a']/span[@class='nav-ftr-text' and contains(text(),'Your Account')]"));
			TimeUnit.SECONDS.sleep(2);
		...
		try {
			WebElement manageBalance = wait.until(ExpectedConditions
					.visibilityOfElementLocated(By.xpath("//span[contains(text(),'Manage gift card balance')]")));
			TimeUnit.SECONDS.sleep(2);
		...
		try {
			WebElement reddem = wait.until(ExpectedConditions
					.visibilityOfElementLocated(By.xpath("//h3[contains(text(),'Redeem a Gift Card')]")));
			TimeUnit.SECONDS.sleep(2);
			...
			try {
				WebElement giftCard = wait
						.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//a[@name='redeemGiftCard']")));
				TimeUnit.SECONDS.sleep(2);
		...
		// {
		// WebElement panel =
		// wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("asv-your-account-balance")));
		// WebElement redeem =
		// panel.findElement(By.xpath("//div/a/div/h3[@class='a-text-bold' and
		// contains(text(),'Redeem')]"));
		// TimeUnit.SECONDS.sleep(2);
		// logger.debug("--->找到[Redeem a Gift Card]");
		// redeem.click();
		// }
		...
		try {
			WebElement input = wait.until(
					ExpectedConditions.visibilityOfElementLocated(By.xpath("//input[@id='gc-redemption-input']")));
			TimeUnit.SECONDS.sleep(2);
		...
		try {
			...
			logger.error("--->对比充值结果");
			TimeUnit.SECONDS.sleep(5);
			WebElement alertSuccess = wait
					.until(ExpectedConditions.visibilityOfElementLocated(By.id("alertRedemptionSuccess")));
		...
		try {
			...
			logger.info("--->{'prev':'" + prev.getText() + "','redeem':'" + redeem.getText() + "','now':'"
					+ now.getText() + "'}");
		} catch (Exception e) {
	...
	public static void main(String[] args) {
		AutoBuy autoBuy = new AmazonUKAutoBuy();
		autoBuy.login("415407279@qq.com", "ws153,.") ;
		autoBuy.cleanCart() ;
		Map<String, String> map = new HashMap<>();
		map.put("url", "http://www.amazon.co.uk/dp/B00TIO8MPO");
		map.put("sku", "[[\"Size\",\"60/13/135\"],[\"Colour\",\"GOLD BLACK/GREY SHADED\"]]");
		/*map.put("url", "https://www.amazon.co.uk/dp/B017CEVWJ4");
		map.put("sku", "[[\"Size\",\"10 UK / 44 EU\"]]");*/
		map.put("num","1") ;
		AutoBuyStatus selectStatus = autoBuy.selectProduct(map);
		if(AutoBuyStatus.AUTO_SKU_SELECT_SUCCESS==selectStatus){
